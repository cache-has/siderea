...
/// # Example
/// ```
/// use astro_core::propagators::keplerian::propagate_state_keplerian;
...
/// # Example
/// ```
/// use astro_core::propagators::keplerian::propagate_lagrange;
/// use astro_core::core::linalg::Vector3;
/// use astro_core::core::constants::GM_EARTH;
///
/// let r0 = Vector3::new(7000e3, 0.0, 0.0);
...
/// Batch propagation of multiple state vectors (Keplerian)
///
/// # Arguments
/// * `states` - Slice of [x, y, z, vx, vy, vz] state vectors (meters, m/s)
/// * `time_steps` - Either a single time step for all states, or one per state (seconds)
/// * `mu` - Standard gravitational parameter (m³/s²)
pub fn batch_propagate_states(
    states: &[[f64; 6]],
    time_steps: &[f64],
    mu: f64,
) -> PoliastroResult<Vec<[f64; 6]>> {
    ...
    states
        ...
        .enumerate()
        .map(|(i, (s, &dt))| {
            let r0 = Vector3::new(s[0], s[1], s[2]);
            let v0 = Vector3::new(s[3], s[4], s[5]);
...
mod tests {
    ...
    fn test_batch_propagate_single_time_step() {
        ...
        let states = [[r1, 0.0, 0.0, 0.0, v1, 0.0], [r2, 0.0, 0.0, 0.0, v2, 0.0]];

        let dt = 3600.0;
        let result = batch_propagate_states(&states, &[dt], GM_EARTH).unwrap();
    ...
    fn test_batch_propagate_multiple_time_steps() {
        ...
        let states = [[r, 0.0, 0.0, 0.0, v, 0.0], [r, 0.0, 0.0, 0.0, v, 0.0]];

        let dt = vec![1800.0, 3600.0];
        let result = batch_propagate_states(&states, &dt, GM_EARTH).unwrap();
    ...
    fn test_batch_propagate_wrong_time_steps() {
        ...
        let result = batch_propagate_states(&states, &dt, GM_EARTH);
        assert!(result.is_err());
    }
    ...
    fn test_batch_propagate_energy_conservation() {
        ...
        let states = [[r0, 0.0, 0.0, 0.0, v0, 0.0], [r0, 0.0, 0.0, 0.0, v0, 0.0]];
    ...
    fn test_batch_propagate_lagrange() {
        ...
        let states = [[r, 0.0, 0.0, 0.0, v, 0.0]];

        let dt = 3600.0;

        let result1 = batch_propagate_states(&states, &[dt], GM_EARTH).unwrap();
        let result2 = batch_propagate_lagrange(&states, &[dt], GM_EARTH).unwrap();
